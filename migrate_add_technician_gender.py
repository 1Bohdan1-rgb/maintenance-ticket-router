"""One-off migration: add technicians.gender.
...
Adds to the "technicians" table:
    - gender   VARCHAR(20)   optional, "male" | "female" (NULL = not
                              specified - technicians are never required to
                              answer this, and existing rows are left as
                              NULL rather than given a guessed value)
...
Usage:
    ...
    python migrate_add_technician_gender.py
"""
import os
import sys
    ...
    try:
        with conn:
            with conn.cursor() as cur:
                cur.execute(
                    "ALTER TABLE technicians ADD COLUMN IF NOT EXISTS gender VARCHAR(20);"
                )
        print("Done: technicians.gender is present.")
    finally:
        conn.close()
